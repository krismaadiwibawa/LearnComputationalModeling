...
stim <- as.matrix(read.table("data/faceStim.csv", sep=",")) # The stimuli used
...
stim <- as.matrix(read.table("data/faceStim.csv", sep=","))

exemplars <- list(a=stim[1:5,], b= stim[6:10,])

data <- scan(file="data/facesDataLearners.txt") # The proportions of 'A' responses by the participants for each stimuli (total 34 stimuli)
data <- ceiling(data*N)
...
nGridsearch <- 50
pb <- progress_bar$new(
  format = "  Running MLE [:bar] :percent | ETA: :eta",
  total = nGridsearch, clear = FALSE, width= 60)
st_c <- runif(nGridsearch, min=0.02, max=9.98)
st_w1 <- runif(nGridsearch, min=0.02, max=0.98)
st_w2 <- runif(nGridsearch, min=0.02, max=0.98)
st_w3 <- runif(nGridsearch, min=0.02, max=0.98)
st_sigma <- runif(nGridsearch, min=0.02, max=9.98)
st_b <- runif(nGridsearch, min=-4.98, max=4.98)
...
for (f in seq_along(st_val)) {
  ...
  #}
#}
  pb$tick() # a command to update progress bar
}
...
#### Try Metropolitan-Hastings for this case
...
n_iter <- 100000
pb <- progress_bar$new(
  format = "  Running MCMC [:bar] :percent | ETA: :eta",
  total = n_iter, clear = FALSE, width= 60)

burnin <- 2000

chain <- matrix(NA, nrow = n_iter, ncol = 6)
colnames(chain) <- c("c", "st_w1", "st_w2", "st_w3", "sigma", "b")

chain[1, ] <- bestres$par # Starting value using the result of MLE
# chain[1, ] <- c(1,1,1,1,1,1)
propsd <- c(0.05, 0.05, 0.05, 0.05, 0.05, 0.05) 

for (i in 2:n_iter) {
  current_theta <- chain[i-1, ]
  proposal <- current_theta + rnorm(6, mean = 0, sd = propsd)

  # manually guardrail from values outside the desired parameter range
  if (any(proposal[1:5] < 0) || proposal[2] > 1 || proposal[3] > 1 || 
      proposal[4] > 1 || proposal[6] < -5 || proposal[6] > 5) {
    chain[i, ] <- current_theta # reject the proposal and fall back to current theta
    next
  }
  
  # Calculate "Scores" (Deviances)
  # Deviance = -2 * log-likelihood. 
  # So, Log-Likelihood = -Deviance / 2
  curr_dev <- GCMutil(current_theta, stim, exemplars, data, N, FALSE)
  prop_dev <- GCMutil(proposal, stim, exemplars, data, N, FALSE)
  
  # Acceptance Step (Log-Space to prevent underflow)
  log_ratio <- (-prop_dev / 2) - (-curr_dev / 2)
  
  if (log(runif(1)) < log_ratio) {
    chain[i, ] <- proposal # Accept
    #cat("Chain ", i, " of ", n_iter,"\n", sep="")
  } else {
    chain[i, ] <- current_theta # Reject
    #cat("Chain ", i, " of ", n_iter, "\n", sep="")
  }
  pb$tick()
}
...
# Throw away burnin
post_burnin_chain <- chain[-(1:burnin), ]
# Take the mean as the parameter estimate
final_params <- colMeans(post_burnin_chain)
...
#### Trying with 1000 observation

```{r}
rm(list=ls())

# 1. Generate Synthetic Data (The "True" population mean is 135)
set.seed(123)
n_obs <- 1000
true_sd <- 15
obs_data <- rnorm(n_obs, mean = 135, sd = true_sd) 

# 2. Setup MCMC
n_iter <- 5000
chain <- rep(NA, n_iter)
chain[1] <- 100 # Starting guess (far from the truth)
propsd <- 0.5   # Tuning: smaller steps because 1000 points create a very "sharp" peak
...
for (i in 2:n_iter) {
  ...
  # Calculate Log-Likelihood for the WHOLE GROUP
  # sum(dnorm(..., log = TRUE)) is the key here
  log_lik_curr <- sum(dnorm(obs_data, mean = current, sd = 15, log = TRUE))
  log_lik_prop <- sum(dnorm(obs_data, mean = proposal, sd = 15, log = TRUE))
  
  # Acceptance Ratio in Log-Space
  log_ratio <- log_lik_prop - log_lik_curr
...
# 4. Results
burnin <- 1000
final_chain <- chain[-(1:burnin)]

dev.new(width = 7, height = 5, noRStudioGD = TRUE)
hist(final_chain, main="Posterior for N=1000", xlab="Estimated Mean IQ")
abline(v = mean(obs_data), col="red", lwd=2) # The data's average
