...
while (abs(ppi-oldppi)>.00001){ # we want the iteration to stop only when the absolute difference (between current iteration ppi and previous iteration ppi) is less than .00001
  ...
  # It generates a list of probabilities (resp ---> responsibility). Some points get a 0.99 (definitely Bell 2), some get a 0.01 (definitely Bell 1), and some in the middle get a 0.5.
  ...
  ppi <- mean(resp) # This is the new Prior for the next round. You are basically saying: "The average of all my individual posterior certainties is now my new 'base rate' for the whole group."
  ...
  # If ppi is 0.72, the model believes that 72% of your saccade data belongs to the second Gaussian distribution (N(150,20)), and the remaining 30% belongs to the first distribution (N(100,10)).
